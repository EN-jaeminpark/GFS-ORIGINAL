...
class Train:
    ...
    def start(self):   
        ...
        for epoch_num in tqdm(range(self.epoch)):
            ...
            print(' ------------- Train Step Epoch Summary ------------------')
            ...
            for batch_num, data in enumerate(self.valid_loader, 0):
                ...
                with torch.no_grad():
                    for item_num in range(price_data.size(0)):
                        ...
                        output = self.model.forward(price,keytrend, self.g2_ctx, 
                        self.valid_graphs[batch_num][item_num])
                ...
                print('Valid Loss for Batch {0} is {1}'.format(batch_num, batch_loss / price_data.size(0)))
                print('Valid ACC for Batch {0} is {1}'.format(batch_num, batch_acc / price_data.size(0)))
                print('Valid MCC for Batch {0} is {1}'.format(batch_num, batch_mcc / price_data.size(0)))
            ...
            final_loss_ = epoch_loss / tot_item
            final_acc_ = epoch_acc / tot_item       
            final_mcc_ = epoch_mcc / tot_item
